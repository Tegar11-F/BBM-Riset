...
function cekKamus($kata) {
    // Ganti konfigurasi koneksi database sesuai dengan ekstensi mysqli
    $host = "localhost";
    $user = "root";
    $password = "";
    $database = "kamus";
    ...
    $query = "SELECT * FROM kamus WHERE `indonesia` = ? LIMIT 1";
...
function Del_Inflection_Suffixes($kata) {
    $inflectionSuffixes = array('/([km]u|nya|[kl]ah|pun)$/', '/([klt]ah|pun)$/');

    foreach ($inflectionSuffixes as $suffix) {
        $kata = preg_replace($suffix, '', $kata, -1, $count);
        if ($count > 0) {
            break;
        }
    }
